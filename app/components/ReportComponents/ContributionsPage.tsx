...
export interface MonetaryContributionRecord {
	id?: string | number;
	user?: ContributionUserInfo | null;
...
interface ContributionsPageProps {
	...
	contributions: MonetaryContributionRecord[];
	pageNumber?: number;
	totalPages?: number;
	isLastPage?: boolean;
}

interface ContributionsPagesProps {
	candidateOrCommitteeName?: string;
	reportStartDate?: string;
	reportEndDate?: string;
	contributions: MonetaryContributionRecord[];
...
function normalizeElectionCycle(contribution: MonetaryContributionRecord) {
...
function fillToFour(items: MonetaryContributionRecord[]) {
	const padded = [...items];
...
function CheckAnonymous({contribution}: {contribution: MonetaryContributionRecord}) {
//grab the list of all contributions
//sum total contributions if great than 2000 return false on wants anonymous for all
//if not group by user id and sum contributions set wants_anonymous to true if sum is less than 100
...
function ContributionBlock({
	...
	contribution: MonetaryContributionRecord;
}) {
	...
	return (
		<div style={styles.contributionBlock}>
			...
			<div style={{ ...styles.row, gap: "9px", marginTop: "0px", marginBottom: "2px" }}>
				<span style={styles.label}>Contribution Received For:</span>
			...
			<div style={styles.row}>
				<span style={styles.label}>Amount of Contribution: $</span>
				<LineField value={formatMoney(contribution.amount)} width="82px" />
				<span style={styles.label}>Date of Contribution:</span>
...
export function paginateContributions(
	...
	for (let index = 0; index < contributions.length; index += itemsPerPage) {
		pages.push(contributions.slice(index, index + itemsPerPage));
	}
...
export const ContributionsPage: React.FC<ContributionsPageProps> = ({
	candidateOrCommitteeName = "Darrin Stout for Sheriff",
	reportStartDate = "",
	reportEndDate = "",
	priorPageTotal,
	contributions,
	...
	const pageContributions = fillToFour(contributions.slice(0, ITEMS_PER_PAGE));
	const pageTotal = contributions.reduce((sum, item) => {
	...
	return (
		<div style={styles.sheet}>
			<div style={styles.bodyWrap}>
				<h1 style={styles.title}>ITEMIZED STATEMENT OF CONTRIBUTIONS - CANDIDATE</h1>
				...
				<div style={{ ...styles.row, marginBottom: "3px" }}>
					<span style={styles.label}>
						3. Total campaign contributions from preceding page (enter $0 if first page) $
					</span>
					<LineField value={precedingDisplay} width="189px" />
				</div>

				<div style={{ marginTop: "25px", marginBottom: "5px", fontWeight: 700, fontSize: "15px" }}>
					COMPLETE THE APPROPRIATE ITEMS FOR EACH ITEMIZED CONTRIBUTION.
				</div>

				<div style={styles.sectionRule} />

				{pageContributions.map((contribution, index) => (
					<ContributionBlock
						key={contribution.id ? String(contribution.id) : `blank-${index}`}
				...
				<div style={{ ...styles.footerWrap,  paddingTop: "5px" }}>
					<div style={styles.row}>
						<span style={{ ...styles.label, fontWeight: 700, marginTop: "10px" }}>Total Contributions: $</span>
						<LineField value={footerTotal} width="278px" />
					</div>
					<p style={styles.footerText}>
						(Carry forward to the next page if additional pages of this form are used. If this is the last page of contributions, this amount must be shown in the summary on first page.)
					</p>
				</div>
			</div>

			<div style={styles.footerBar}>
				<span>SS-1131 (Rev. 1/2023)</span>
...
export const ContributionsPages: React.FC<ContributionsPagesProps> = ({
	candidateOrCommitteeName = "Darrin Stout for Sheriff",
	reportStartDate = "",
	reportEndDate = "",
	contributions,
	...
	const pages = paginateContributions(contributions, ITEMS_PER_PAGE);
	...
	return (
		<>
			{pages.map((pageItems, index) => {
				const page = (
					<ContributionsPage
						key={`contributions-page-${index + 1}`}
