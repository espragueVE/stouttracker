...
export const FormsManager: React.FC<FormsManagerProps> = ({ forms, donors, onSaveLogEntry, formFields = {} }) => {
  ...
  const [newUserData, setNewUserData] = useState({ firstName: '', lastName: '', email: '', phone: '' });
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-black text-slate-900">Campaign Documentation</h2>
          <p className="text-slate-600 text-sm mt-1">Log activities and fill out required reporting forms</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {forms.map(form => (
          <div key={form.id} className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col hover:shadow-md transition-shadow group">
            <div className="p-6 flex-1">
              ...
              <p className="text-sm text-slate-500 mb-4 line-clamp-3">{form.description}</p>
            </div>
        ...
        <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl p-6 flex flex-col items-center justify-center text-center space-y-3 min-h-[280px]">
           ...
           <div>
            <p className="text-sm font-bold text-slate-600">Custom Form Builder</p>
            <p className="text-xs text-slate-500">Create specialized tracking logs</p>
           </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center sticky top-0 bg-white rounded-t-xl">
              <div>
                <h3 className="text-xl font-black text-slate-900">{selectedForm.title}</h3>
                <p className="text-xs text-slate-500 font-semibold uppercase tracking-widest mt-1">New Log Entry</p>
              </div>
              <button 
            ...
            <div className="p-6 space-y-6">
              {/* User Selection */}
              <div>
                <h4 className="font-bold text-slate-900 mb-3 uppercase text-sm tracking-wide">Select Participant</h4>
                ...
                {!isNewUser ? (
                  <div className="space-y-3">
                    <select 
                      ...
                      <option value="">Choose from existing participants...</option>
                      {donors.map(donor => (
              ...
              <div>
                <h4 className="font-bold text-slate-900 mb-3 uppercase text-sm tracking-wide">Required Information</h4>
