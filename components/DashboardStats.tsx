...
export const DashboardStats: React.FC<DashboardStatsProps> = ({ donors }) => {
  ...
  const stats = useMemo(() => {
    ...
    const sortedByDate = [...donors].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    ...
    const chartData = Array.from(dailyDataMap.entries()).map(([name, amount]) => ({ name, amount })).slice(-7);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          ...
          <div className="text-2xl font-bold text-slate-900">${stats.totalRaised.toLocaleString()}</div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <div className="flex items-center justify-between pb-2">
            <p className="text-sm font-medium text-slate-500">Avg. Contribution</p>
            <TrendingUp className="h-4 w-4 text-sheriff-600" />
          </div>
          <div className="text-2xl font-bold text-slate-900">${Math.round(stats.avgDonation)}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <h3 className="text-lg font-semibold text-slate-800 mb-4">Fundraising Trend</h3>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.chartData}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12}} dy={10} />
                <YAxis axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12}} tickFormatter={(value: number) => `$${value}`} />
                <Tooltip 
                  cursor={{fill: '#f1f5f9'}} 
        ...
        {/* Pie Chart */}
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <h3 className="text-lg font-semibold text-slate-800 mb-4">Supporter Demographics (Age)</h3>
